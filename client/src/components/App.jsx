...
class App extends React.Component {
  ...
  getReviews() {
    ...
    $.get(`http://3.21.164.148:5001/restaurants/100?${this.state.sort}`, (results) => {
      this.setState({
        data: results,
      });
    });
  ...
  getTotalReviews() {
    $.get('http://3.21.164.148:5001/reviews/100', (results) => {
  ...
  selectNextPage() {
    ...
    $.get(`http://3.21.164.148:5001/restaurants/100?start=${start}&${this.state.sort}`, (results) => {
      this.setState({
        data: results,
      });
    });
  ...
  searchReviews(value) {
    ...
    $.get(`http://3.21.164.148:5001/restaurants/100?${this.state.sort}&q=${value}`, (results) => {

      this.setState({
        data: results,
      });
    });
    $.get(`http://3.21.164.148:5001/reviews/100?q=${value}`, (results) => {
  ...
  sortHandler(value) {
    ...
    if (value === '') {
      ...
      $.get(`http://3.21.164.148:5001/restaurants/100?${sortQuery}`, (results) => {
        this.setState({
          data: results,
          sort: sortQuery,
          totalReviews: this.state.initialReviews,
  ...
  updateVote(vote, reviewInfo) {
    ...
    $.ajax(`http://3.21.164.148:5001/review/${reviewInfo.review_id}?value=${voteType}&voted=${voteNum}`, {
      type: 'PATCH',
      data: reviewInfo,
      success: (result) => {
